...
if $fMRI_A; then
	...
	# 2) flags_PhysiolReg_aCompCorr=false - mean WM and CSF signal regression
		...
		if ${flags_PhysiolReg_aCompCorr}; then  ### if using aCompCorr
			export flags_PhysiolReg_WM_CSF=false   # make sure only one flag is 'on'
		...
		if ${flags_PhysiolReg_WM_CSF}; then   ### if using mean WM and CSF signal reg
		...
		if ${flags_NuisanceReg_AROMA}; then  
			...
			if ${flags_PhysiolReg_aCompCorr}; then  
				...
				export regPath="AROMA/NoTissueReg"
			fi          
			...
			if ${flags_PhysiolReg_aCompCorr}; then  
				...
				export regPath="HMPreg/NoTissueReg"
			fi          
		fi

		export flags_EPI_GS=true # global signal regression 

		## WARNING if no tissue regression is performed, set configs_EPI_numGS = 0
