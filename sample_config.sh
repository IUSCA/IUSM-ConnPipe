...
export pathSM="/N/project/cfn-rady/ConnPipe/ConnPipelineSM"

################################################################################
############################  PATH TO RAW DATA  ################################

# USER INSTRUCTIONS- PLEASE SET THIS PATH TO POINT TO YOUR DATA DIRECTORY
export path2data="/N/project/cfn-rady/ConnPipe/TestData"

################################################################################
############################  PATH TO DATA DERIVATIVES #########################

# USER INSTRUCTIONS- PLEASE SET THIS PATH TO POINT TO YOUR OUTPUT DIRECTORY
export path2derivs="/N/project/cfn-rady/ConnPipe/Derivatives"	
...
export configs_sefmFolder="UNWARP1" # Reserved for Spin Eco Field Mapping series
	...
	# 3) flags_NuisanceReg="AROMA_HMP": apply ICA-AROMA followed by HMPreg. 
		...
		export flags_NuisanceReg="AROMA_HMP"
			...
			if [[ ${flags_NuisanceReg} == "AROMA" ]] || [[ ${flags_NuisanceReg} == "AROMA_HMP" ]]; then 
				...
				# set to false if AROMA has been succesfully run before. AROMA ouput folder must exist inside EPI output folder.
