...
paths.data = '/N/project/YoderLab/NANSTAN/Datadir';
...
if ~exist('subjectList','var')
    ...
    subjectList = dir(fullfile(paths.data,'CS0*'));
end

% subjectList(end)=[];
clear subjectList %remove the above generated list
subjectList(1).name = 'CS004'; 
            ...
            for j=1:length(epiList)
                ...
                if j >= configs.EPI.epiMin && j <= configs.EPI.epiMax
                    paths.EPI.dir=fullfile(paths.subject,epiList(j).name);
                        ...
                        [paths,flags,configs,parcs]=f_fMRI_plots_new(paths,flags,configs,parcs);
